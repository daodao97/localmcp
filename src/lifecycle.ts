...
const socketFile = resolve(stateDir, 'agent.sock');
...
export function request(command = 'status'): Promise<Status> {
  return new Promise((resolveStatus, reject) => {
    ...
    socket.on('end', () => {try {const response = JSON.parse(data); if (response.error) reject(new Error(response.error)); else resolveStatus(response);} catch (error) {reject(error);}});
  });
}
...
export async function serveControl(getStatus: () => Status, stop: () => void, reload: () => Promise<void>) {
  ...
  await new Promise<void>((ready, reject) => {server.once('error', reject); server.listen(socketFile, ready);});
  await chmod(socketFile, 0o600);
  return async () => {server.close(); await rm(socketFile, {force: true});};
}
